...
if test -e $IPbam && test -s $IPbam ; then
    if test ! -e $mdir/$prefix.resultfile ; then Rscript $R -c=$IPbam -i=$IPbam -rf -savn=$mdir/$prefix.narrowPeak -savr=$mdir/$prefix.regionPeak -out=$mdir/$prefix.resultfile -savp=$mdir/$prefix.pdf; fi
